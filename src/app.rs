...
use crate::provider::{
    calendar_request, parse_answer, physics_request, Answer, Anthropic, Chain, Gemini, Ollama,
    OpenAi, OpenAiCompat, Provider, Shot,
};
use crate::router;
...
/// and must reconstruct the `Box` to free it.
pub const WM_APP_ROUTER_RESULT: u32 = WM_APP + 12;
    ...
    if SETTINGS_OPEN.with(|c| c.get()) {
        ...
        match settings_reentrancy_policy(msg, taskbar_created_msg) {
            SettingsReentrancy::Ignore => {
                // WM_APP_LEARNED, WM_APP_CALENDAR_RESULT, WM_APP_PALETTE_RUN
                // and WM_APP_ROUTER_RESULT are the only ignored messages
...
mod tests {
    ...
    use super::{WM_APP_ACTIVATE, WM_APP_CALENDAR_RESULT, WM_APP_RESULT, WM_APP_ROUTER_RESULT};
